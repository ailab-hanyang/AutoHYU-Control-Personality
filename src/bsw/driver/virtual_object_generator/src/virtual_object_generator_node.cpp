...
VirtualObjectGenerator::VirtualObjectGenerator(std::string task_node, double period):
    ...
    if ( nh.getParam("/virtual_object_generator/csv_path", csv_path_string) ) {
    ...
    if ( nh.getParam("/virtual_object_generator/csv_noise_path", csv_noise_path_string) ) {
...
int main(int argc, char** argv) {
    // Initialize node
    std::string node_name = "virtual_object_generator";
    ros::init(argc, argv, node_name);
    ros::NodeHandle nh;

    // Get and period
    double period;
    if ( !nh.getParam("task_period/period_virtual_object_generator", period) ) {
        period = 1.0;
    }
