...
print(
    "config has", c.listLength(), "list entries. (should be 0 because it is a map now.)"
)

r = rviz.YamlConfigReader()
c = rviz.Config()
r.readFile(c, roslib.packages.get_pkg_dir("rviz") + "/autohyu.rviz")
if r.error():
    print("Error:", r.errorMessage())
else:
    print("autohyu.rviz first two levels of maps:")
    mi = c.mapIterator()
    while mi.isValid():
        print("key:", mi.currentKey(), " value:", mi.currentChild().getValue())
        mi2 = mi.currentChild().mapIterator()
    ...
    print("Entire autohyu.rviz written to a string:")
    print(first_string)
