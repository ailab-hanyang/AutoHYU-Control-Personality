...
#ifndef __MORAI_VEHICLE_DRIVER_HPP__
#define __MORAI_VEHICLE_DRIVER_HPP__
#pragma once
...
class VehicleDriver : public TaskManager {
    ...
    private:
        ...
        inline void CallbackMRIMU(const sensor_msgs::Imu::ConstPtr& msg) {
            std::lock_guard<std::mutex> lock(mutex_mr_imu_);
            i_mr_imu_ = GetMRIMU(*msg);
            is_initialized = true;
        }
        inline void CallbackMRGPS(const morai_msgs::GPSMessage::ConstPtr& msg) {
            std::lock_guard<std::mutex> lock(mutex_mr_gps_);            
            i_mr_gps_ = GetMRGPS(*msg);
            is_initialized = true;
        }
        inline void CallbackMRVehicleStatus(const morai_msgs::EgoVehicleStatus::ConstPtr& msg) {
            std::lock_guard<std::mutex> lock(mutex_mr_vehicle_status_);
            i_mr_vehicle_status_ = GetMRVehicleStatus(*msg);
            is_initialized = true;
        }

        VehicleState GetMRVehicleStatus(const morai_msgs::EgoVehicleStatus& msg);
        Gnss GetMRGPS(const morai_msgs::GPSMessage& msg);
        Motion GetMRIMU(const sensor_msgs::Imu& msg);
        ...
        // Subscriber
        Subscriber s_mr_gps_;
        Subscriber s_mr_imu_;
        Subscriber s_mr_ego_vehicle_status_;
        ...
        Gnss i_mr_gps_;
        Motion i_mr_imu_;
        VehicleState i_mr_vehicle_status_;

        mutex mutex_mr_imu_;
        mutex mutex_mr_gps_;
        mutex mutex_mr_vehicle_status_;
        ...
        MRVehicleDriverConfig cfg_;
        bool is_initialized = false;
};

#endif  // __MORAI_VEHICLE_DRIVER_HPP__