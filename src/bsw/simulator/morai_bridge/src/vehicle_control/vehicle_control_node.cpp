...
VehicleControl::VehicleControl(std::string node_name, double period)
    ...
    // Publisher init
    p_control_signal_ = nh.advertise<morai_msgs::CtrlCmd>(
            "ctrl_cmd", 10);
...
void VehicleControl::ProcessINI() {
    if (util_ini_parser_.IsFileUpdated()) { 
        util_ini_parser_.ParseConfig("Morai Vehicle control", "use_keyboard_input", cfg_.use_keyboard_input);  
...
std::tuple<double, double, double, double> VehicleControl::CalculateCommand(const VehicleState& vehicle_state, 
    ...
    if(command_torque >= morai_ioniq5::T_NEUTRAL) {       
        gas = min(1.0, morai_ioniq5::GAS_1*command_torque);
        brake = 0.0;
    }
    else if(command_torque < morai_ioniq5::T_NEUTRAL) {
        gas = 0.0;
        brake = min(1.0, (-1.)*morai_ioniq5::BRAKE_1*command_torque);
    }
    ...
    return std::make_tuple(front_tire_steer_deg*DEG2RAD, gas, brake, command_accel);
}
...
void VehicleControl::UpdateCommand(const double& steer, const double& gas, const double& brake, const double& accel) {
    // Update command    
    o_control_signal_.longlCmdType  = 1;
    o_control_signal_.steering      = steer;
    o_control_signal_.accel         = gas;
