...
int Map::ClosestWaypoint(double x, double y, double search_boundary){
    static int previous_closest_index = 0; // 초기값은 유효하지 않은 값으로 설정

    double closest_len = 100000;
    int closest_waypoint = 0;

    if(search_boundary > 0.99){
        for (int i = 0; i < maps_x_.size(); i++)
        {
            double map_x = maps_x_[i];
            double map_y = maps_y_[i];
        ...
        previous_closest_index = closest_waypoint;
    }
...
// @junhee lee 241106 : find closest waypoint based on previous closest index
std::vector<double> Map::ToFrenet(double x, double y, double search_boundary) {
	int wp1 = ClosestWaypoint(x, y, search_boundary);
