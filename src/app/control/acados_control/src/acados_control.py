...
# MPC Models
from MPC.NMPC_boundary.NMPC_class_kinematic import Nonlinear_Model_Predictive_Controller as Model_Predictive_Controller_Kinematic
...
class AcadosControl(object):
    ...
    def __init__(self):
        ...
        # Parameters (reading ini)
        self.controller_params['personality'] = {}
        
        cfg = configparser.ConfigParser()
        script_dir = os.path.dirname(os.path.realpath(__file__))
        ini_file_path = os.path.join(script_dir, "../../../../../config/personality.ini")
        print(ini_file_path)
        with open(ini_file_path, 'r', encoding='utf-8-sig') as f:
            cfg.read_file(f)
        
        self.controller_params['personality']['ax_min'] = cfg['PersonalityExtraction']['ax_min']
        self.controller_params['personality']['ax_max'] = cfg['PersonalityExtraction']['ax_max']
        self.controller_params['personality']['ay_max'] = cfg['PersonalityExtraction']['ay_max']
        self.controller_params['personality']['jx_min'] = cfg['PersonalityExtraction']['jx_min']
        self.controller_params['personality']['jx_max'] = cfg['PersonalityExtraction']['jx_max']
        self.controller_params['personality']['jy_max'] = cfg['PersonalityExtraction']['jy_max']
        self.controller_params['personality']['max_speed'] = 70.0 / 3.6

        # Predictions : Every controller's output must satisfy ... [x,y,yaw,vx,vy,r,delta,trq]
        self.target_pred = np.zeros((self.N_MPC, 8)) 
        self.sampled_trajectory = np.zeros((self.N_MPC+1, 5)) # [t, x, y, yaw, speed]
    ...
    def loop(self):
        ...
        while not rospy.is_shutdown():
            ...
            ego_trajectory = transform_traj_to_ego(vehicle_state, trajectory) # Ego frame trajectory, [time, x, y, z, yaw, curvature, distance, speed, acceleration]
            
            frenet_n, sampled_trajectory = get_ref_trajectory_ds(ego_trajectory, self.N_MPC+1, self.Ts_MPC) # [t, x, y, yaw, speed, curvature, s]
            self.sampled_trajectory = sampled_trajectory
            if self.MPC_kinematic is None:
                self.MPC_kinematic = Model_Predictive_Controller_Kinematic(
                    self.config_path,
            ...
            self.target_pred, target_input, states = self.MPC_kinematic.calculate_optimal(sampled_trajectory, frenet_n, vehicle_state, self.MPC_params, self.controller_params)
