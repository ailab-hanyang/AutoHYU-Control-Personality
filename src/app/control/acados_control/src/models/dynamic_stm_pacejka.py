...
def pred_stm(veh_params_file, tire_params_file = "pacejka_params_file.yaml"):   
    ...
    model_name  = "dynamic_bicycle_model"
    # load vehicle params
    ...
    ## Dynamics Definition
    vlong = if_else(vlong > 3.0, vlong, 3.0)
    ...
    s_dot    = (vlong * cos(mu) - vlat * sin(mu))/(1.0 - n*kapa_ref)
    n_dot    = (vlong * sin(mu) + vlat * cos(mu))/s_dot
    mu_dot     = yawrate/s_dot - kapa_ref
    # Source: EuroRacing
    vlong_dot   = (1 / m *(Fx_r - Faero - Fy_f * sin(delta_f) + Fx_f * cos(delta_f) - Fbanking_x + m * vlat * yawrate)) /s_dot
    vlat_dot    = (1 / m *(Fy_r + Fy_f * cos(delta_f) + Fx_f * sin(delta_f) - Fbanking_y - m * vlong * yawrate)) /s_dot
    yawrate_dot = (1 / Iz * (lf * (Fy_f * cos(delta_f) + Fx_f * sin(delta_f)) - lr * Fy_r)) /s_dot
