...
class AcadosControl(object):
    ...
    def __init__(self):
        ...
        with open(self.config_path+self.controller_params['veh_params_file_MPC'], 'r') as file:
            self.veh_params = yaml.load(file, Loader=yaml.FullLoader)
        self.veh_params_update_flag = True
    ...
    def loop(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.MPC_kinematic is None or self.veh_params_update_flag == True:
                ...
                self.veh_params_update_flag = False
            self.update_params()
            self.update_veh_params()
            
            self.target_pred, target_input, states = self.MPC_kinematic.calculate_optimal(sampled_trajectory, frenet_n, vehicle_state, self.MPC_params)
    ...
    def update_veh_params(self):        
        with open(self.config_path+self.controller_params['veh_params_file_MPC'], 'r') as file:
            veh_params = yaml.load(file, Loader=yaml.CLoader)
        if veh_params != self.veh_params:
            self.veh_params = veh_params
            self.veh_params_update_flag = True
            print("Vehicel params updated ! Reinitialize Acados solver")
