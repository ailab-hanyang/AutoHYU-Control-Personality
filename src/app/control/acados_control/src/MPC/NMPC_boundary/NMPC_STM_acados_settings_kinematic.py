...
import models.dynamic_stm_pacejka
...
def acados_settings(Tf, N, x0, Q, R, mode, L1_pen, L2_pen, ax_max_interpolant, ay_max_interpolant, combined_acc_limits, controller_params, veh_params_file = "veh_params_pred.yaml", tire_params_file= "pacejka_params.yaml", solver_generate_C_code = True, solver_build = True):
    ...
    if mode == "kinematic":
        ...
        pred_model, constraints = models.dynamic_stm_pacejka.pred_stm(veh_params_file, tire_params_file)
    ...
    vel_cost = if_else( x[3] <  1, 1.0 , x[3]) # adjust for negative angles
    cost_x_expr = vertcat(x[1], yaw) # 0 으로 수렴해야하는 것들
    ocp.model.cost_expr_ext_cost   = 0.5 * cost_x_expr.T @ mtimes(Q[1:3,1:3], cost_x_expr) + 0.5 * u.T @ mtimes(R, u) + Q[3,3]/(vel_cost *vel_cost)
    ocp.model.cost_expr_ext_cost_e = cost_x_expr.T @ mtimes(Q[1:3,1:3], cost_x_expr) # + Q[3,3]/(vel_cost *vel_cost)

    # cost_x_expr = vertcat(x[1], yaw, x[3]-p[1])
    # ocp.model.cost_expr_ext_cost   = 0.5 * cost_x_expr.T @ mtimes(Q[1:4,1:4], cost_x_expr) + 0.5 * u.T @ mtimes(R, u) 
    # ocp.model.cost_expr_ext_cost_e = cost_x_expr.T @ mtimes(Q[1:4,1:4], cost_x_expr) # + Q[3,3]/(vel_cost *vel_cost)
    ...
    ocp.constraints.lbx     = np.array(
        [-1,
         0, 
         float(controller_params['personality']['ax_min'])])
    ocp.constraints.ubx     = np.array(
        [1, 
         float(controller_params['personality']['max_speed']), 
         float(controller_params['personality']['ax_max'])]) 
    ocp.constraints.idxbx   = np.array([1, 3, 7])   # indices of bounds on x (defines Jbx) at intermediate shooting nodes (1 to N-1)
    # s n mu vx vy r delta a 
    ocp.constraints.Jsbx    = np.eye(3) 

    # fff
    ocp.constraints.lbu     = np.array([float(controller_params['personality']['jx_min'])])
    ocp.constraints.ubu     = np.array([float(controller_params['personality']['jx_max'])])
    ...
    # --- lateral jerk
    jerk_lat = 2*x[3]*x[6]*x[7]/3.0 + x[3]**2*u[1]/3.0  # 2*vlong*deltaf*a / 3 + vlong^2*steerrate / 3
    ...
    ocp.constraints.lh      = np.array(
        [-float(controller_params['personality']['ay_max']),
         -float(controller_params['personality']['jy_max'])])
    ocp.constraints.uh      = np.array(
        [float(controller_params['personality']['ay_max']),
         float(controller_params['personality']['jy_max'])])  # upper bound for nonlinear inequalities at shooting nodes (0 to N-1)
    
    # ocp.constraints.lh_e    = np.array([0.0, -3]) 
    # ocp.constraints.uh_e    = np.array([14.5, 3]) 
    ...
    # create solver
    file_name = "acados_ocp_"+mode+".json"
