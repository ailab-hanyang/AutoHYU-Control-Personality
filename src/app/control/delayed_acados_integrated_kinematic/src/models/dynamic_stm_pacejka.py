...
def pred_stm(veh_params_file, tire_params_file = "pacejka_params_file.yaml"):   
    ...
    def init_state(vehicle_state, prediction, prev_empty, another_model_state = None):
        if(prev_empty == True):
            ...
            print("dynamic model MPC get init")
        else:
            X0 = np.array([0.0, 0.0, 0.0, vehicle_state.vx , vehicle_state.vy, vehicle_state.yaw_rate, prediction[1,6], prediction[1,7]]) 

        if another_model_state is not None:
            X0 = np.array([0.0, 0.0, 0.0, vehicle_state.vx , vehicle_state.vy, vehicle_state.yaw_rate, another_model_state[1,4], another_model_state[1,5]]) 
        return X0
