...
def acados_settings(Tf, N, x0, Q, R, Qe, L1_pen, L2_pen, ax_max_interpolant, ay_max_interpolant, combined_acc_limits, veh_params_file = "veh_params_pred.yaml", tire_params_file= "pacejka_params.yaml", solver_generate_C_code = True, solver_build = True):
    ...
    # ocp.constraints.lbx     = np.array([pred_model.delta_f_min, pred_model.a_min]) # lower bounds on x at intermediate shooting nodes (1 to N-1)
    # ocp.constraints.ubx     = np.array([pred_model.delta_f_max, pred_model.a_max]) # upper bounds on x at intermediate shooting nodes (1 to N-1)
    # ocp.constraints.idxbx   = np.array([6, 7])   # indices of bounds on x (defines Jbx) at intermediate shooting nodes (1 to N-1)
    # ocp.constraints.lbu     = np.array([pred_model.jerk_min, pred_model.delta_f_dot_min])
    # ocp.constraints.ubu     = np.array([pred_model.jerk_max, pred_model.delta_f_dot_max])
    # ocp.constraints.idxbu   = np.array([0, 1])
    ...
    # create solver
    acados_solver = AcadosOcpSolver(ocp, json_file="acados_ocp_dynamic_stm.json", generate=solver_generate_C_code, build=solver_build)#, verbose=False)
