...
class Nonlinear_Model_Predictive_Controller:
    ...
    def __init__(
        ...
        X0_MPC = np.array([0.0, 0.0, 0.0, reference_trajectory[0, 4], 0.0, 0.0, 0.0, 0.0] )
        ...
        if costfunction_type == 'NONLINEAR_LS':
            from MPC.NMPC_linear_ls.NMPC_STM_acados_settings_dynamic import acados_settings      
        (
            self.constraint,
            self.model,
            self.acados_solver,
    ...
    def solve(
        ...
        for j in range(self.N):
            # print(self.costfunction_type)
            if self.costfunction_type == 'NONLINEAR_LS':
                yref = np.array([current_ref_traj[j, 1], current_ref_traj[j, 2], current_ref_traj[j, 3], current_ref_traj[j, 4], 0, 0, 0, 0, 0, 0]) 
                self.acados_solver.set(j, "yref", yref)
                
            if self.costfunction_type == 'EXTERNAL':
                yref = np.array([current_ref_traj[j, 1], current_ref_traj[j, 2], current_ref_traj[j, 3], current_ref_traj[j, 4]])
                self.acados_solver.set(j, "p", yref)

            if self.costfunction_type == 'LINEAR_LS' :
                yref = np.array([current_ref_traj[j, 1], current_ref_traj[j, 2], current_ref_traj[j, 3], current_ref_traj[j, 4], 0, 0, 0, 0, 0, 0]) 
                self.acados_solver.set(j, "yref", yref)
                # print(j, "ref :", yref)
                

        yref_N = np.array([current_ref_traj[self.N, 1], current_ref_traj[self.N, 2], current_ref_traj[self.N, 3], current_ref_traj[self.N, 4], 0, 0, 0, 0])
        if self.costfunction_type == 'NONLINEAR_LS':
    ...
    def reintialize_solver(self, X_ref, solver_generate_C_code=False, solver_build=False):
        # [t, x, y, yaw, speed]
        x0 = np.array([0.0, 0.0, 0.0, X_ref[0, 4], 0.0, 0.0, 0.0, 0.0] )
        ## --- Load MPC formulation ---
        costfunction_type = self.MPC_params['costfunction_type']
        if costfunction_type == 'NONLINEAR_LS':
            from MPC.NMPC_linear_ls.NMPC_STM_acados_settings_dynamic import acados_settings        
        self.constraint, self.model, self.acados_solver, self.ocp = acados_settings(self.Tp, self.N, x0,  
    ...
    def calculate_optimal(self, X_ref, vehicle_state, MPC_params):
