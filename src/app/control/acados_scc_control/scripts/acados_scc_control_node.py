...
class AcadosSCCControl(object):
    ...
    def __init__(self):
        # init ROS node
        rospy.init_node('acados_scc_control', anonymous=True)
        ...
        self.N_MPC = 40 # Number of MPC Steps
        self.Ts_MPC = 0.1 # Time Step for MPC
    ...
    def loop(self):
        ...
        while not rospy.is_shutdown():
            ...
            # Vehicle State: [x, y, yaw, speed]
            # Measured State: [x, y, yaw, speed, s, n, mu]
            vehicle_state, measured_state = npize_vehicle_state(self.i_vehicle_state)
            
            # target_trajectory: [x, y, yaw, speed]
            # reference_map: [x, y]
            # right_boundary: [x, y, yaw]
            # left_boundary: [x, y, yaw]
            target_trajectory, reference_map, right_boundary, left_boundary = npize_trajectory(self.i_bhv_trajectory)
            
            # objects: [x, y, yaw, id, True, height, length, width]
            objects         = npize_objects(self.i_objects,max_objects_num=1)
            
            # Generate Road Map (Frenet) from reference_map
            road_map = generate_road_map(reference_map)
            
            ## Frenet Transform
            # Add s, n to vehicle_state[x, y, yaw, speed, s, n]
            vehicle_state  = frenet_transform(road_map,vehicle_state)
            # Add mu to vehicle_state[x, y, yaw, speed, s, n, mu]
            vehicle_state  = frenet_mu_transform(road_map,vehicle_state)

            # Resample points with delta time (self.Ts_MPC) and sample_num (self.N_MPC)
            target_traj_resampled = resample_points(target_trajectory, vehicle_state[0,4], self.Ts_MPC, self.N_MPC)
            # Frenet Transform: Determine s, n
            target_traj_resampled_frenet_sn = frenet_transform(road_map,target_traj_resampled)
            # Add mu to trajectory[x, y, yaw, speed, s, n, mu]
            target_traj_resampled_frenet = frenet_mu_transform(road_map, target_traj_resampled_frenet_sn, start_mu=vehicle_state[0,6])

            right_boundary_frenet = frenet_transform(road_map,right_boundary)
            right_boundary_frenet = frenet_mu_transform(road_map, right_boundary_frenet, start_mu=vehicle_state[0,6])

            left_boundary_frenet = frenet_transform(road_map,left_boundary)
            left_boundary_frenet = frenet_mu_transform(road_map, left_boundary_frenet, start_mu=vehicle_state[0,6])

            ## Filtering objects in lane
            # Convert objects to frenet coordinate
            sn_objects = np.zeros((np.shape(objects)[0], np.shape(objects)[1], 11)) # [x,y,yaw,id,valid,height,length,width,s,n]
            for i in range(np.shape(objects)[0]):
                if objects[i,:,4].all() == True: # Valid object
                    object = objects[i, :, :]
                    object_sn = frenet_transform(road_map,object)
                    object_sn = frenet_mu_transform(road_map,object_sn)
                    if np.isnan(object_sn).any():
                        object_sn[np.isnan(object_sn)] = 0  # NaN인 위치에 0 할당
                    sn_objects[i, :, :] = object_sn
                else:
                    sn_objects[i, :, :] = np.zeros((np.shape(objects)[1], 11)) # [x,y,yaw,id,valid,height,length,width,s,n]
            # Filtering objects in lane
            objects_frenet = filtering_objects_in_lane(sn_objects,left_boundary_frenet,right_boundary_frenet)

            end = time.perf_counter() # MPC Preprocessing Time

            target_cart, self.target_pred, target_input, states = self.MPC.calculate_optimal(road_map,
                                       vehicle_state,
                                       objects_frenet,
                                       target_traj_resampled_frenet,
                                       right_boundary_frenet,
                                       left_boundary_frenet,
                                       measured_state)
            ...
            self.o_command_torque.data = np.float32(target_input[1])

            # Update Resample Path
            self.update_resample_path(target_traj_resampled_frenet)
            # Update Control Path
            self.update_control_path(target_cart)
            # Update Control Info
            total_time_end = time.time()           
            total_time = 1000.0 * (total_time_end - total_time_start)
            self.update_control_info(vehicle_state, target_traj_resampled_frenet, total_time, states)
            # Publish Things
            self.publish()
    ...
    def publish(self):
        ...
        self.p_command_torque.publish(self.o_command_torque)
...
def main():
    ...
    main function
    """
    acados_scc_control = AcadosSCCControl()
    try:
        acados_scc_control.loop()
    finally:
        if acados_scc_control is not None:
            acados_scc_control.destroy()
