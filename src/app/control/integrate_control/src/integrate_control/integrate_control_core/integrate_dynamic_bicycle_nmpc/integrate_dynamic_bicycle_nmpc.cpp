#include <integrate_control/integrate_control_core/integrate_dynamic_bicycle_nmpc/integrate_dynamic_bicycle_nmpc.hpp>

IntegrateDynamicBicycleNMPC::IntegrateDynamicBicycleNMPC() :ControllerInterface() {   
    ...
    ProcessINI();
    std::cout << "init IntegrateDynamicBicycleNMPC" << std::endl;

    solver_ = std::make_unique<IntegrateDynamicBicycleNMPCAcadosWrapper>(step_dt_);
    step_num_ = INTEGRATE_DYNAMIC_BICYCLE_NMPC_N+1;
}

IntegrateDynamicBicycleNMPC::~IntegrateDynamicBicycleNMPC() {
}

void IntegrateDynamicBicycleNMPC::BuildControllerVehicleState(const VehicleState& vehicle_state, tk::Map road_map){
    // std::cout << "IntegrateDynamicBicycleNMPC : " << (int)live_cnt_++ << std::endl;
...
boost::optional<ControlCommand> IntegrateDynamicBicycleNMPC::CalculateOptimalCommand(const ControlTrajectory& ref){
    ...
    if (i_vehicle_state_.vehicle_can.operation_mode != OperationMode::AUTONOMOUS || 
        ...
        for(int i = 0; i<opt_vals.size() ; i++){
            ControlPoint point = {opt_vals[i].first[0], opt_vals[i].first[1], opt_vals[i].first[2],  0, opt_vals[i].first[3]};

            control_command.control_trajectory.control_point.push_back(point);
        }
        if(opt_vals.size() >= 1){
            control_command.steering_tire_angle = opt_vals[1+params_.shift_input_step].first[6]*RAD2DEG;
            control_command.ax = opt_vals[1+params_.shift_input_step].first[7];
            control_command.trq = control_command.ax * VEHICLE_MASS * VEHICLE_WHEEL_RADIUS;
        }
        else{
...
vector<IntegrateDynamicBicycleNMPC::OptVariables> IntegrateDynamicBicycleNMPC::SolveAcados(const ControlTrajectory& ref){

    // set initial state [x, y, yaw, vx, vy, yawrate, delta, a]
    State x0;
    if(optimal_states_.empty()){
        x0 = {0.0, 0.0, 0.0,
             i_vehicle_state_.vx, i_vehicle_state_.vy , i_vehicle_state_.yaw_vel,
             i_vehicle_state_.vehicle_can.steering_tire_angle, i_vehicle_state_.ax};
    }
    else{
        x0 = {0.0, 0.0, 0.0, 
              i_vehicle_state_.vx, i_vehicle_state_.vy , i_vehicle_state_.yaw_vel,
              optimal_states_.at(1)[6], optimal_states_.at(1)[7]};
    }
    ...
    for (int i = 0; i < step_num_; ++i) { // Indices: 0 to 39
        ...
        std::vector<double> x_guess = {
            ...
            0.0, //-ref.control_point.at(i).curvature*ref.control_point.at(i).vx,
            0.0,
            0.0
        };
    ...
    vector<IntegrateDynamicBicycleNMPC::OptVariables> opt_vals;
    if (opt_status == ACADOS_SUCCESS)
    {
        ...
        for (int i=0; i<step_num_; ++i) { // Indices: 1 to 39
            IntegrateDynamicBicycleNMPC::OptVariables opt_var;
...
void IntegrateDynamicBicycleNMPC::CheckSolvedStateTwisted(const ControlTrajectory& ref, const ControlCommand& control_command){
    // Calculate errors
    // 1. distance error
    // 2. yaw error
...
void IntegrateDynamicBicycleNMPC::ProcessINI() {
    if (util_ini_parser_.IsFileUpdated()){
        util_ini_parser_.ParseConfig("Integrate Dynamic Bicycle NMPC", "step_dt",               params_.step_dt             );
        util_ini_parser_.ParseConfig("Integrate Dynamic Bicycle NMPC", "shift_input_step",      params_.shift_input_step    );
        util_ini_parser_.ParseConfig("Integrate Dynamic Bicycle NMPC", "weight_x",              params_.weight_x            );
        util_ini_parser_.ParseConfig("Integrate Dynamic Bicycle NMPC", "weight_y",              params_.weight_y            );
        util_ini_parser_.ParseConfig("Integrate Dynamic Bicycle NMPC", "weight_yaw",            params_.weight_yaw          );
        util_ini_parser_.ParseConfig("Integrate Dynamic Bicycle NMPC", "weight_speed",          params_.weight_speed        );
        util_ini_parser_.ParseConfig("Integrate Dynamic Bicycle NMPC", "weight_steer",          params_.weight_steer        );
        util_ini_parser_.ParseConfig("Integrate Dynamic Bicycle NMPC", "weight_accel",          params_.weight_accel        );
        util_ini_parser_.ParseConfig("Integrate Dynamic Bicycle NMPC", "weight_x_end",          params_.weight_x_end        );
        util_ini_parser_.ParseConfig("Integrate Dynamic Bicycle NMPC", "weight_y_end",          params_.weight_y_end        );
        util_ini_parser_.ParseConfig("Integrate Dynamic Bicycle NMPC", "weight_yaw_end",        params_.weight_yaw_end      );
        util_ini_parser_.ParseConfig("Integrate Dynamic Bicycle NMPC", "weight_speed_end",      params_.weight_speed_end    );
        util_ini_parser_.ParseConfig("Integrate Dynamic Bicycle NMPC", "weight_steer_end",      params_.weight_steer_end    );
        util_ini_parser_.ParseConfig("Integrate Dynamic Bicycle NMPC", "weight_accel_end",      params_.weight_accel_end    );
        util_ini_parser_.ParseConfig("Integrate Dynamic Bicycle NMPC", "weight_dsteer",         params_.weight_dsteer       );
        util_ini_parser_.ParseConfig("Integrate Dynamic Bicycle NMPC", "weight_jerk",           params_.weight_jerk         );

        util_ini_parser_.ParseConfig("Integrate Dynamic Bicycle NMPC", "avg_distance_error_max",params_.avg_distance_error_max  );
        util_ini_parser_.ParseConfig("Integrate Dynamic Bicycle NMPC", "avg_yaw_error_max",     params_.avg_yaw_error_max       );

        step_dt_ = params_.step_dt;
        cout << "Update Integrate Dynamic Bicycle NMPC ini" << endl;
