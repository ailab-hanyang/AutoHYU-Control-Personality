#ifndef __INTEGRATE_KINEMATIC_BICYCLE_NMPC_HPP__
#define __INTEGRATE_KINEMATIC_BICYCLE_NMPC_HPP__
#pragma once
...
#include <integrate_control/integrate_control_core/integrate_control_interface.hpp>
...
class IntegrateKinematicBicycleNMPC: public ControllerInterface{	
	public:
        explicit IntegrateKinematicBicycleNMPC();
        virtual ~IntegrateKinematicBicycleNMPC();

    public:

        void ProcessINI();		
		void BuildControllerVehicleState(const VehicleState& vehicle_state, tk::Map road_map) override;
	   	boost::optional<ControlCommand> CalculateOptimalCommand(const ControlTrajectory& ref) override;
	...
	private:
        ...
        // acados
        std::unique_ptr<IntegrateKinematicBicycleNMPCAcadosWrapper> solver_;
	    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - //
	    // Functions
		...
		IntegrateKinematicBicycleNMPCParams params_;
...
#endif  // __INTEGRATE_KINEMATIC_BICYCLE_NMPC_HPP__
