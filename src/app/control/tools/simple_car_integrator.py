...
# 오일러 방식으로 차량 모델 업데이트 함수 추가
def euler_update(state, delta, a, dt=0.02):
    x, y, yaw, vx, vy, yawrate = state
    ...
    # Lateral Behavior
    alpha_f = delta - np.arctan2((vy + lf * yawrate), vx) if vx > 0.001 else 0.0
    alpha_r = np.arctan2((lr * yawrate - vy), vx) if vx > 0.001 else 0.0

    # Lateral Tire Forces (Pacejka 'magic formula')
    Fy_f = Df * np.sin(Cf * np.arctan(Bf * alpha_f - Ef * (Bf * alpha_f - np.arctan(Bf * alpha_f))))
    Fy_r = Dr * np.sin(Cr * np.arctan(Br * alpha_r - Er * (Br * alpha_r - np.arctan(Br * alpha_r))))

    # 동역학 방정식 정의 (다이나믹 자전거 모델)
    dx = vx * np.cos(yaw) - vy * np.sin(yaw)
    dy = vx * np.sin(yaw) + vy * np.cos(yaw)
    dyaw = yawrate
    dvx = (Fx_r - Faero - Fy_f * np.sin(delta) + m * vy * yawrate) / m
    dvy = (Fy_r + Fy_f * np.cos(delta) - m * vx * yawrate) / m
    dyawrate = (lf * Fy_f * np.cos(delta) - lr * Fy_r) / Iz
    ...
    return np.array([x, y, yaw, vx, vy, yawrate])

# RK4 방식으로 차량 모델 업데이트 함수 추가
def rk4_update(state, delta, a, dt=0.02):
    def derivatives(state, delta, a):
        x, y, yaw, vx, vy, yawrate = state
        ...
        return np.array([dx, dy, dyaw, dvx, dvy, dyawrate])

    k1 = derivatives(state, delta, a)
    k2 = derivatives(state + 0.5 * dt * k1, delta, a)
    k3 = derivatives(state + 0.5 * dt * k2, delta, a)
    k4 = derivatives(state + dt * k3, delta, a)
...
steering_increment = np.deg2rad(10)   # 조향각 변화량 per frame [rad]
...
instruction_text = [
    ...
    "M : Toggle Model (CasADi / Euler / RK4)"
]

# 키 입력 방법을 터미널에 노란색으로 출력
print(Fore.YELLOW + "\n".join(instruction_text) + Style.RESET_ALL + "\n")

# 모델 선택 변수 (0: CasADi, 1: Euler, 2: RK4)
model_selection = 0
...
while running:
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_SPACE:
                ...
                # 모델 토글 (CasADi / Euler / RK4)
                model_selection = (model_selection + 1) % 3
    ...
    if model_selection == 0:
        ...
        state_k = euler_update(state_k, delta_current, a_current)
    elif model_selection == 2:
        state_k = rk4_update(state_k, delta_current, a_current)
    ...
    # 현재 모델 표시
    model_text = "Model: CasADi" if model_selection == 0 else "Model: Euler" if model_selection == 1 else "Model: RK4"
