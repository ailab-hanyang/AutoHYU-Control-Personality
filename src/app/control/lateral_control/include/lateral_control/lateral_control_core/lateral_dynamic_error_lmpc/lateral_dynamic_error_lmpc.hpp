#ifndef __LATERAL_DYNAMIC_ERROR_HPP__
#define __LATERAL_DYNAMIC_ERROR_HPP__
#pragma once
...
class LateralDynamicErrorLMPC: public LateralControllerInterface{	
	public:
        explicit LateralDynamicErrorLMPC();
        virtual ~LateralDynamicErrorLMPC();
	...
	private:
		static constexpr int	dim_state_ = 5;
		...
		State GetContinuousModelDeriv(const State &x,const Input &u, const double &v, const double &curvature);
		State GetReferecneState(const ControlTrajectory& ref, const uint32_t& idx);
		State GetInitState(bool is_prev_state_empty);
		LinModelMatrix GetDiscreteLinModelMatrix(const State &x, const Input &u, const double &v, const double &curvature);
		...
		FrenetVehicleState i_vehicle_frenet_state_;
		...
		LateralDynamicErrorLMPCParams params_;
