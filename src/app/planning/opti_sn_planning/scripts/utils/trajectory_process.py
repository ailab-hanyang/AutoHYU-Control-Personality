...
import math
import time
from opti_sn_planning import libfrenet as frenet
from geometry_msgs.msg import Point, Pose, Quaternion

def npize_vehicle_state(vehicle_state):
    x0 = np.zeros((1, 4)) # x,y,yaw,speed,s,n,mu
...
def npize_trajectory(trajectory):
    point           = np.zeros((len(trajectory.point), 4))          # x,y,yaw,speed,s,n,mu
    reference_map   = np.zeros((len(trajectory.reference_map), 5))  # x,y,yaw,s,curvature
    right_boundary  = np.zeros((len(trajectory.right_boundary), 3)) # x,y,yaw,s,n,mu
    left_boundary   = np.zeros((len(trajectory.left_boundary), 3))  # x,y,yaw,s,n,mu
