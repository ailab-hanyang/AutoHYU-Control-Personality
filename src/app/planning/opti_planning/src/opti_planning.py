...
class GoalVectorControl(object):
    """
    Control With Acados
    """
    ...
    def __init__(self):
        # init ROS node
        rospy.init_node('opti_planning', anonymous=True)
        ...
        self.s_trajectory = rospy.Subscriber(
            "app/pla/behavior_trajectory", BehaviorTrajectory, self.callback_trajectory)
        self.is_trajectory = False

        self.s_predicted_objects = rospy.Subscriber(
            "app/perc/predicted_objects", PredictObjects, self.callback_objects)
        ...
        self.i_trajectory = BehaviorTrajectory()
        self.i_objects = np.zeros((1, 31, 2))
        ...
        self.sampled_trajectory = np.zeros((self.N_MPC+1, 5)) 
    ...
    def loop(self):
        ...
        while not rospy.is_shutdown():
            ...
            objects = self.i_objects

            ego_boundary = transform_boundary_to_ego(vehicle_state, trajectory)
            # print(np.shape(ego_boundary))
            ego_trajectory = transform_traj_to_ego(vehicle_state, trajectory)
            ego_objects = transform_object_to_ego(vehicle_state, objects)
            ...
            self.update_params()
            self.target_pred, target_input, states = self.MPC_kinematic.calculate_optimal(ego_boundary, sampled_trajectory, vehicle_state, self.MPC_params, ego_objects)
    ...
    def callback_objects(self, data):
        objects_np = self.convert_objects_to_numpy(data)
        
        if objects_np.shape == (0,):
            pass
        else:
            self.i_objects = objects_np
    ...
    def convert_objects_to_numpy(self, predict_objects_msg):
        objects_list = []
        for obj in predict_objects_msg.object:
            states_np = np.array([[state.x, state.y] for state in obj.state])
            objects_list.append(states_np)
...
def main():
    ...
    main function
    """
    opti_planning = GoalVectorControl()
    try:
        opti_planning.loop()
    finally:
        if opti_planning is not None:
            opti_planning.destroy()
